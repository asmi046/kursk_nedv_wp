...
<?php
/*
Template Name: Страница Дома, участки, дачи
...
<main class="page">
	...
	<?
		...
		$object = $wpdb->get_results( "SELECT * FROM `kn_objnedv`  WHERE (`type` = 'Часть дома' OR `type` = 'Земельный участок' OR `type` = 'Дом' OR `type` = 'Дача'  OR `type` = 'Гараж') ".$sparam." LIMIT ".$ofset.", ".$countInPage );

		$objectUl = $wpdb->get_results( "SELECT * FROM `kn_objnedv`  WHERE (`type` = 'Часть дома' OR `type` = 'Земельный участок' OR `type` = 'Дом' OR `type` = 'Дача'  OR `type` = 'Гараж') ".$sparam.";"  );
		...
		<div class="container">
			...
			<div class="product-info__row d-flex">
				...
				<div class="product-info__column">
					...
					<?php 
						$param = array("curentpage" => $curentPage, 'pagecount' => $pageCount, "prefix" => "doma-uchastki-dachi");
